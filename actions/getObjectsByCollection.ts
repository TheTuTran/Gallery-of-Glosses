 ...
 */
const getObjectsByCollection = async (collectionName: string): Promise<ListObject> => {
    const queryObj = {
        "$or": [
    ...
    while (true) {
        try {
            const response = await axios.post(`https://tinydev.rerum.io/app/query?limit=${limit}&skip=${skip}`, queryObj, {
                headers: {
                    'Content-Type': 'application/json; charset=utf-8'
                }
            });

            listObj.itemListElement = listObj.itemListElement.concat(response.data.map((item: any) => ({ '@id': item.target ?? item["@id"] ?? item.id })));
            
            if (response.data[0]) {
                listObj["@type"] = response.data[0]["@type"] ?? response.data[0].type ?? "ItemList";
            }
...
export default getObjectsByCollection;
