 ...
 */
const getObjectsByValue = async (targetId: string): Promise<ObjectData[]> => {
    let queryObj: { [key: string]: any } = {}; 
    let targetConditions: { [key: string]: string }[] = [];

    if (targetId.startsWith('http')) {
        let httpVersion = targetId.startsWith('https') ? `http${targetId.slice(5)}` : `https${targetId.slice(4)}`;
        let httpsVersion = targetId.startsWith('https') ? `https${targetId.slice(5)}` : `http${targetId.slice(4)}`;
...
export default getObjectsByValue;
