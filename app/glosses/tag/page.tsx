...
export default function TagPage() {
    ...
    const fetchObjectsByCollection = async () => {
        try {
            ...
            if(objectsFromCollection.itemListElement){
                const objectIds = objectsFromCollection.itemListElement.map((item) => item['@id']); 
                ...
                for (let i = 0; i < objectIds.length; i++) {
                    await getObjectsByValue(objectIds[i]).then(obj => {
                        let glossData = obj.map(item => {
                            if (item.body && typeof item.body === 'object' && item.body[0] !== '') {
    ...
    const handleBrowseClick = () => {
        const filteredGlosses = glosses.filter(gloss => gloss.tags.items.includes(selectedTag));
        ...
        <div className="flex gap-4 p-8">
            ...
            <div className="h-fit min-h-screen bg-white rounded-md p-8 border-gold border overflow-auto flex gap-4">
                ...
                <div className="w-[25%] bg-gray-100 p-4 rounded-md">
                    ...
                    <p className="py-2">
                        Various glosses share certain features or terms. We have selectively ascribed tags to capture this 
                        information. Here you may browse according to term or feature tag and see all the glosses that have 
                        shared content in this way. Tags will show up as the glosses with tags get rendered in
                    </p>
                    <select className="mb-2 border-2 border-gray-200 rounded-sm w-full p-2 px-3" onChange={(e) => setSelectedTag(e.target.value)}>
                        <option value="">Select a Tag</option>
                        {allTags.map((tag, index) => 
                            <option key={index} value={tag}>{tag}</option>
                        )}
                    </select>
                    <button className="hover:bg-primaryHover shadow-sm bg-primary text-white px-4 rounded-md py-2 items-center gap-1" onClick={handleBrowseClick}>Browse by this tag</button>
                    {isLoading && 
